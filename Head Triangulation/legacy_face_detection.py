...
class faceDetection():
    ...
    The eye detection algorithm should also be passed as object or function to the
    """
    ...
    def __call__(self, img, camera=0, eye="right"):
        ...
        if len(previous_faces) == 0: # No previous face detection, using slow but accurate detection on whole image
            predictions = self.face_detector.process(img)
            ...
            if faces_detected:
                for face in predictions.detections:
                    ...
                    faces.append([   int(face.location_data.relative_bounding_box.xmin*img_width),
                    ...
                    # rough eye estimates provided by the face_detection model
                    self.eye_estimates.append([face.location_data.relative_keypoints[0].x * img_width,
                                              face.location_data.relative_keypoints[0].y * img_height])
            ...
            for prediction, patch, (x_offset, y_offset) in zip(predictions, face_patches, face_offsets):
                ...
                if prediction.detections:
                    for face in prediction.detections:
                        ...
                        self.eye_estimates.append([x_offset + face.location_data.relative_keypoints[0].x * patch_width,
                                                   y_offset + face.location_data.relative_keypoints[0].y * patch_height])
...
if __name__=="__main__":
    ...
    fd = faceDetection(use_eye_detection=False)
